...
function [estvar,ivar] = falco_est_batch(DM,mp,ep,ivar,modvar,estvar)
...
wi_ref = mp.wi_ref;
Nsbp = mp.Nsbp;
sbp_center_vec = mp.sbp_center_vec;
corr_inds = mp.F4.full.corr.inds; %--IMPORTANT: Images are from full model but control is on the compact model, so resolutions at F4 must be same for compact and full models.
score_inds = mp.F4.full.score.inds; %--IMPORTANT: Images are from full model but control is on the compact model, so resolutions at F4 must be same for compact and full models.

% Itr = estvar.Itr;
Npairs = ep.Npairs; % % Number of image PAIRS for DM Diversity or Kalman filter initialization
% IcubeOut = zeros(mp.F4.full.Neta,mp.F4.full.Nxi,1+2*Npairs);
% VcubeOut = zeros(mp.F4.full.Neta,mp.F4.full.Nxi,2,1+2*Npairs);
...
for si=1:Nsbp   %--MOVE THIS OUTSIDE THIS FUNCTION #2CHANGE
...
%% Compute probe shapes and take probed images:
    ...
    modvar.whichImg = 1;
%     [Iunprobed,ivar] = hcil_getImage(DM_config, ivar, mp, modvar);
    [Iunprobed,ivar] = falco_get_image(mp, modvar, DM,ivar);
...
% %     VcubeOut(:,:,1, modvar.whichImg) = DM.dm1.V;
% %     VcubeOut(:,:,2, modvar.whichImg) = DM.dm2.V;
    ...
    for qq=1:2*Npairs
        ...
        ProbeH=falco_est_gen_probe_surf(ep.ProbeArea,mp.Ddm,...
            sbp_center_vec(si),probePhaseVec(qq),ep.ProbeoffsetX,ep.ProbeoffsetY,XS,YS,InormProbe); %Function used to compute the Probe Shape. Output is in meters.
        ...
        if(ep.whichDMprobe == 1)
            dDM1Vprobe = dDMVprobe./DM.dm1.VtoH; % Now in volts
            dDM2Vprobe = 0;
        elseif(ep.whichDMprobe == 2)
            dDM1Vprobe = 0;        
            dDM2Vprobe = dDMVprobe./DM.dm1.VtoH; % Now in volts
        end
%         dDM2Vprobe = dDM2Vprobe.*padarray(ones(32),[5,5],0);
        % cd(folders.m); fn = sprintf('map_probe%d_DM1.fits',qq); fitswrite(dDM1Vprobe,fn);
        DM.dm1.V = DM1Vnom+dDM1Vprobe;
        DM.dm2.V = DM2Vnom+dDM2Vprobe;

        modvar.whichImg = 1+qq;
        %[Im,ivar] = hcil_getImage(DM_config, ivar, folders, mp, modvar);
        [Im,ivar] = falco_get_image(mp, modvar, DM, ivar);
...
DM.dm1.V = DM1Vnom;
DM.dm2.V = DM2Vnom; % Added July 9, 2014
Eunprobed = falco_model_compact(mp, DM_config,modvar);
...
for qq=1:Npairs
    % For plus probes:
    DM.dm1.V = squeeze( DM1Vplus(:,:,qq));
    DM.dm2.V = squeeze( DM2Vplus(:,:,qq));
    Etemp = falco_model_compact(mp, DM_config, modvar);
    Eplus(:,qq) = Etemp(corr_inds);
    % For minus probes:
    DM.dm1.V = squeeze( DM1Vminus(:,:,qq));
    DM.dm2.V = squeeze( DM2Vminus(:,:,qq));
    Etemp = falco_model_compact(mp, DM_config, modvar);
...
% estvar.timeTotal = Nsbp*(1+2*Npairs)*ivar.ExpTime*ivar.num_im; % Since unprobed imaged is half that time.
...
%     cd(folders.data)
%     %fnI = sprintf('img_run%03d_estItr%03d_im%04dto%04d.fits',ivar.RunNum,estvar.Itr,ImNum0,ivar.ImNum);
%     %fnV = sprintf('dmV_run%03d_estItr%03d_im%04dto%04d.fits',ivar.RunNum,estvar.Itr,ImNum0,ivar.ImNum);
